...
async function categorizeBatch(routes, productName, batchNum, totalBatches) {
  console.log(`\nProcessing batch ${batchNum}/${totalBatches} (${routes.length} routes)...`);

  const screenList = routes.map(r => `${r.id} | "${r.name}" | ${r.path}`).join('\n');

  const systemPrompt = `You are a senior product designer organizing ${productName} app screens into meaningful user flows.
...
- Keep flow names concise (2-4 words)
...
You MUST include ALL ${routes.length} screens in your response.`;
  ...
  try {
    const message = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Categorize these ${routes.length} ${productName} screens into logical user flows. Each line starts with the UUID:\n\n${screenList}`
      }]
    });
...
async function run() {
  ...
  if (!productId) {
    console.error('Usage: node categorize-flows-generic.js <product-id>');
    process.exit(1);
  }
  ...
  console.log(`Categorizing flows for: ${product.name}`);

  // Fetch all routes for this product
  ...
  console.log(`Found ${routes.length} screens.`);
  ...
  for (let i = 0; i < batches.length; i++) {
    const result = await categorizeBatch(batches[i], product.name, i + 1, batches.length);
    if (result) {
  ...
  for (const [flowName, screens] of Object.entries(flowGroups)) {
    console.log(`  ${flowName} (${screens.length} ${screens.length === 1 ? 'screen' : 'screens'})`);
  }
